...
**  \brief Тест памяти
**	
**	Приложение тестирует память на базовом модуле,
**	используя псевдослучайную последовательность или 64-битный счетчик.
**  При запуске программы без параметров используется псевдослучайная последовательность,
**  тест выполняется на всех найденных модулях и повторяется бесконечное чмсло раз,
**  он может быть прерван по нажатию Ctrl+C.
**  С помощью параметров командной строки можно изменить поведение программы:
**		-cnt	  - будет использоваться 64-битный счетчик
**		-b <lid>  - тест будет выполняться только на модуле с указанным LID
**		-r <num>  - тест будет повторятся num раз
**
** (C) InSys, 2020
**
*/
